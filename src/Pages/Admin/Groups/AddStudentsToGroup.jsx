...
import { XCircle, Search, LoaderCircle, ChevronLeft, ChevronRight } from "lucide-react";
import { getAllUsers } from "../../../Controllers/userControllers";
import { addStudentsToGroup } from "../../../Controllers/groupController";

const AddStudentsToGroup = ({ groupId, existingStudentIds = [], onClose, onSuccess }) => {
  const [allStudents, setAllStudents] = useState([]); // all fetched students
  const [students, setStudents] = useState([]);       // filtered for search
  ...
  const fetchStudents = async (page = 1) => {
    try {
      ...
      setAllStudents(filtered);  // store all fetched students
      setStudents(filtered);     // initially show all
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center px-4">
      <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-lg relative border border-indigo-100">
            ...
            {students.map((stu) => (
              ...
              >
                ...
                <span className="text-sm">
                  {stu.name} <span className="text-xs text-gray-500">({stu.email})</span>
                </span>
              </label>
            ))}
          </div>
        ...
        {/* Footer row with pagination and buttons */}
