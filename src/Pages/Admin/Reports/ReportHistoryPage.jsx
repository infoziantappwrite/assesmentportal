        ...
        <div className="max-w-6xl mx-auto p-6">
            ...
            {loading ? (
                ...
                <div className="bg-gray-50 rounded-xl p-8 text-center">
                    ...
                    <p className="text-gray-500">Your exported reports will appear here</p>
                </div>
            ) : (
                <div className="space-y-3">
                    {history.map((entry) => {
                        ...
                        const isExpired = file_info?.expires_at && new Date() > new Date(file_info.expires_at);
                                    ...
                                    <div className="space-y-2 flex-1">
                                        ...
                                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-600">
                                            ...
                                            {isCompleted && (
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span>Completed: {formatDateTime(completed_at)}</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        {isCompleted && (
                                            <div className="pt-1">
                                                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                                                    ...
                                                    <span>Expires: {formatDateTime(file_info?.expires_at)}</span>
                                                    {downloaded_at && (
                                        ...
                                        {isFailed && (
                                            <div className="flex items-start gap-2 mt-1 p-2 bg-red-50 rounded text-red-700 text-sm">
                                                ...
                                                <span>{processing?.error_message || "Report generation failed"}</span>
                                            </div>
                                        )}
                                    </div>
